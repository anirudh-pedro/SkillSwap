...
const EducationStep = () => {
      ...
      <AnimatePresence>
        {showSuggestions && (
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className="fixed bg-slate-900 border border-slate-700 rounded-md shadow-lg overflow-y-auto z-[9999] text-slate-100"
            style={{
          ...
          >
            {institutionSuggestions.map((institution, index) => (
              <motion.div
                key={index}
                className="px-4 py-2 cursor-pointer hover:bg-cyan-900/40 text-sm text-slate-100 border-b border-slate-800 last:border-b-0 truncate"
                onClick={() => handleInstitutionSelect(institution)}
                whileHover={{ backgroundColor: "rgba(8, 145, 178, 0.15)" }}
              >
  ...
  return (
    <motion.div
      ...
      exit="exit"
      variants={fadeIn}
      className="flex flex-col space-y-6 py-2 relative z-10 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-800 min-h-screen text-slate-100"
    >
      <motion.div variants={headingVariants}>
        <div className="flex items-center mb-6">
          <motion.div 
            ...
            className="w-10 h-10 bg-cyan-900 rounded-full flex items-center justify-center mr-3"
          >
          ...
          <div>
            <h2 className="text-2xl font-semibold text-slate-100 tracking-tight">Educational Background</h2>
            <p className="text-sm text-slate-300 mt-1">Tell us about your academic qualifications</p>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div
        ...
        className="bg-slate-900 rounded-xl p-6 shadow-sm border border-slate-800"
      >
        <motion.div 
        ...
        >
          <Input
            ...
            className="transition-all duration-300 focus:ring-2 focus:ring-cyan-400 text-slate-100 bg-slate-800 border-slate-700"
          />
        </motion.div>
        ...
        >
          <div className="relative" onClick={focusInput}>
            <label htmlFor="collegeName" className="block text-sm font-medium text-slate-200 mb-1">
            ...
            <div className="relative flex">
              <input
                id="collegeName"
                type="text"
                className="w-full px-3 py-2 border border-slate-700 rounded-md shadow-sm
                           focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:border-cyan-400
                           transition-all duration-300 text-slate-100 bg-slate-800"
              ...
              <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                {isLoading ? (
                  ...
                  >
                    <svg className="w-5 h-5 text-slate-400 hover:text-cyan-400" viewBox="0 0 20 20" fill="currentColor">
            ...
            <p className="mt-1 text-xs text-slate-400">The institution where you received your highest degree</p>
      ...
      <motion.div
        variants={formFieldVariants}
        className="bg-cyan-900/30 rounded-lg p-4 border border-cyan-800 flex items-center mt-1"
      >
